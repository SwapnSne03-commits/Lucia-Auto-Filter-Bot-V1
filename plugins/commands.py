...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        silenxbotz=await message.reply_sticker("Wʜʏ Dɪsᴛᴜʀʙɪɴɢ Mᴇ !!😋")
    ...
    if data.startswith("selectedfiles"):
        ...
        if len(parts) == 3:
            ...
            return await message.reply('<b><i>Invalid link!</i></b>')

        scoped_key = f"{key}_{message.from_user.id}"
        selected_files = temp.SELECTED.get(scoped_key)
        if not selected_files:
            return await message.reply('<b><i>No files selected or session expired!</i></b>')
    ...
    if not files_:
        ...
        try:
            ...
            k = await msg.reply(f"<b>⚠️ Tʜɪꜱ Fɪʟᴇ Wɪʟʟ Aᴜᴛᴏ Dᴇʟᴇᴛᴇ Fʀᴏᴍ Hᴇʀᴇ Aꜰᴛᴇʀ {get_time(DELETE_TIME)},\n\nSᴏ Fᴏʀᴡᴀʀᴅ Tʜɪs Fɪʟᴇ Tᴏ Yᴏᴜʀ Sᴀᴠᴇᴅ ᴍsɢ Oʀ Sᴏᴍᴇᴡʜᴇʀᴇ Eʟsᴇ Aɴᴅ Sᴛᴀʀᴛ Dᴏᴡɴʟᴏᴀᴅ Tʜᴇʀᴇ.</b>", quote=True)
            await asyncio.sleep(DELETE_TIME)
            await msg.delete()
